...
def clean(frames, background_training_iterations = 5, dish_radius_buffer = 10):
    backsub = fish.train_background_subtractor(frames, iterations=background_training_iterations)
    ...
    dish_mask = cv.circle(dish_mask, (dish.x, dish.y), dish.r + dish_radius_buffer, 1, thickness = -1)
...
if __name__ == "__main__":
    ...
    for movie in movies:
        ...
        output_frames = clean(input_frames, background_training_iterations = 1, dish_radius_buffer = 5)

        op = fish.make_movie(
            OUT / f"{movie}__cleaned.mp4", frames=output_frames, num_frames=len(input_frames), fps=10
        )
